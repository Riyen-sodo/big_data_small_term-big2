...
void TableManager::load(string filePath, bool readAll, int &blockNum, int blockOffset) {
    try {
        ...
        int n_block;
        {
            ...
            rewind(fp);
            n_block = ceil((1. * file_size) / (1. * BLOCK_SIZE));
            blockNum = n_block;
        }
        ...
        for (int i = 0; i < n_block; i++) {
            ...
            int n_item;
            {
                n_item = (block.phd.free_space_start - PageHeaderData::get_size()) / (Item::get_size());
...
void TableManager::save_tuple_datas(string filePath) {
    try {
        int offset = 0;
        for (const Block &block:this->blocks) {
            Writer writer;
            int n_tuple = block.tuple_datas.size();
            WRITE_EXTERN_BASIC_TYPE(int, n_tuple, offset);
            for (const TupleData &tupleData:block.tuple_datas) {
                WRITE_EXTERN_BASIC_TYPE(int, tupleData.n_attr, offset);
                for (const string &str:tupleData.datas) {
                    // 写入大小
